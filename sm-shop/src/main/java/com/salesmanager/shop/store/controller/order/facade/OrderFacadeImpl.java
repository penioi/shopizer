...
public class OrderFacadeImpl implements OrderFacade {
    ...
    public void updateOrderProducts(Order order, List<PersistableOrderProduct> changedOrderProducts, MerchantStore merchantStore) {
        ...
        if (changedOrderProducts.isEmpty()) {
            return;
        }
        try {
            ShoppingCart shoppingCart = shoppingCartService.getByCode(order.getShoppingCartCode(), merchantStore);
            if (shoppingCart == null) {
                throw new ResourceNotFoundException("ShoppingCart with code [" + order.getShoppingCartCode() + "] not found");
            }
            // get order products
            ...
            for (OrderProduct orderProduct : orderProducts) {
                PersistableOrderProduct persistableOrderProduct = changedOrderProducts.stream().filter(o -> o.getId()
                        .equals(orderProduct.getId())).findFirst().orElse(null);
                ShoppingCartItem shoppingCartItem = shoppingCart.getLineItems().stream()
                        .filter(o -> o.getSku().equals(orderProduct.getSku()))
                        .findFirst().orElseThrow(() -> new ResourceNotFoundException("OrderProduct with sku [" + orderProduct.getSku() + "] not found"));
                if (persistableOrderProduct != null) {
                    ...
                    ProductAvailability availability = p.getAvailabilities().iterator().next();
                    if (availability.getProductQuantity() < deltaQtty) {
                        throw new ServiceException(ServiceException.EXCEPTION_INVENTORY_MISMATCH, "Product with sku [" + persistableOrderProduct.getSku() + "] not available in the requested quantity");
                    }
                    ...
                    shoppingCartItem.setSubTotal(persistableOrderProduct.getPrice().multiply(new BigDecimal(persistableOrderProduct.getOrderedQuantity())));
                }
                shoppingCartItem.setAdminAdjusted(true); // lock the price
            }
            ...
            shopOrder.setShoppingCartItems(new ArrayList<>(shoppingCart.getLineItems()));
            shopOrder.setCartCode(shoppingCart.getShoppingCartCode());
            Customer customer = initEmptyCustomer(merchantStore);
            ReadableCustomer readableCustomer = customerFacade.getCustomerById(order.getCustomerId(), merchantStore, null);
            Billing billing = new Billing();
            billing.setCity(readableCustomer.getBilling().getCity());
            billing.setCountry(new Country(readableCustomer.getBilling().getCountry()));
            billing.setFirstName(readableCustomer.getBilling().getFirstName());
            billing.setLastName(readableCustomer.getBilling().getLastName());
            billing.setPostalCode(readableCustomer.getBilling().getPostalCode());
            billing.setTelephone(readableCustomer.getBilling().getPhone());
            billing.setAddress(readableCustomer.getBilling().getAddress());
            billing.setZone(new Zone(billing.getCountry(), readableCustomer.getBilling().getZone(), null));
            customer.setBilling(billing);
            OrderTotalSummary orderTotalSummary = this.calculateOrderTotal(merchantStore, customer, shopOrder, shoppingCart.getMerchantStore().getDefaultLanguage());
            System.out.println("orderTotalSummary = " + orderTotalSummary);
            List<com.salesmanager.core.model.order.OrderTotal> newTotals = new ArrayList<>();
            for (com.salesmanager.core.model.order.OrderTotal total : orderTotalSummary.getTotals()) {
                com.salesmanager.core.model.order.OrderTotal orderTotal = order.getOrderTotal().stream().filter(o -> o.getOrderTotalType().equals(total.getOrderTotalType())).findFirst().orElseThrow(() -> new ResourceNotFoundException("OrderTotal with code [" + total.getOrderTotalCode() + "] not found"));
                orderTotal.setValue(total.getValue());
                newTotals.add(orderTotal);
            }
            order.getOrderTotal().addAll(newTotals);
            order.getOrderTotal().retainAll(newTotals);
        } catch (Exception e) {
            throw new ServiceRuntimeException("Error while updating order product", e);
        }
        try {
            orderService.update(order);
        } catch (ServiceException e) {
            throw new ServiceRuntimeException("Error while updating order", e);
        }
    }
